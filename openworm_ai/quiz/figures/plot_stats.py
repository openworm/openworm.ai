...
def forest_plot(summary, detailed):
    ...
    for ax, cat, colour, ci_colour, label in [
        ...
        errors = np.array(
            [
                [max(0, d - lo), max(0, hi - d)]
                for d, lo, hi in zip(deltas, ci_los, ci_his)
            ]
        ).T

        # Plot
        colours_bar = [
            COLOURS["positive"] if d >= 0 else COLOURS["negative"] for d in deltas
        ]
        ax.barh(
            y_pos,
            deltas,
    ...
    fig.suptitle(
        "Per-Model RAG Impact with 95% Bootstrap CIs",
        fontsize=14,
        fontweight="bold",
        y=0.98,
    )
...
def mcnemar_mosaic(detailed):
    ...
    fig.suptitle(
        "McNemar's Test: RAG Fixed vs Broke (Paired Question Outcomes)",
        fontsize=13,
        fontweight="bold",
        y=1.02,
    )
...
def bootstrap_distributions(detailed):
    ...
    for cat, colour, label in [
        ...
        ax.hist(
            boot_deltas, bins=60, alpha=0.5, color=colour, label=label, density=True
        )
    ...
    ax.set_title(
        "Bootstrap Distribution of RAG Improvement\n(10,000 resamples)",
        fontsize=13,
        fontweight="bold",
    )
...
def effect_size_comparison(summary):
    ...
    comparisons = [
        (
            "Our RAG\n(Corpus Qs)",
            our_effects.get("C. elegans (Corpus)", 0),
            COLOURS["corpus"],
        ),
        ...
        (
            ...
            0.85,
            COLOURS["literature"],
        ),  # ~18% improvement, estimated d
        (
            "BiomedRAG\nWang+ 2025",
            0.55,
            COLOURS["literature"],
        ),  # ~10% improvement, estimated d
        (
            "BrainGPT\nLuo+ 2024",
            0.30,
            COLOURS["literature"],
        ),  # +3pp from tuning, estimated d
    ...
    for threshold, label, y_offset in [
        (0.2, "Small", -0.03),
        (0.5, "Medium", -0.03),
        (0.8, "Large", -0.03),
    ]:
    ...
    ax.set_title(
        "Effect Size Comparison: Our RAG vs Published RAG/Domain Systems",
        fontsize=13,
        fontweight="bold",
    )
    ...
    lit_patch = mpatches.Patch(
        color=COLOURS["literature"], alpha=0.8, label="Published benchmarks (est.)"
    )
